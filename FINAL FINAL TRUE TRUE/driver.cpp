...
int main() {
	...
	int direction;
	int winFlag;
	TStackPtr moveStack = NULL;
	TStackPtr moveUndone = NULL;
	...
	while (!glfwWindowShouldClose(window)) {
		...
		//glfwSetCursorPosCallback(window, cursor_position_callback); //UNCOMMENT TO DISPLAY MOUSE POSITION ON MOUSE MOVE
		...
		if (stateMouse == GLFW_PRESS && oldStateMouse == GLFW_RELEASE) {
			...
			if (isSpaceEmpty(direction, xposEnd, yposEnd, indexOfFirstPositionVecOfSelectedBlock, vertices)) {
				...
				case 1:
					push(&moveStack, createPosStack(indexOfFirstPositionVecOfSelectedBlock,
						vertices[indexOfFirstPositionVecOfSelectedBlock].position,
						vertices[indexOfFirstPositionVecOfSelectedBlock + 1].position,
						vertices[indexOfFirstPositionVecOfSelectedBlock + 2].position,
						vertices[indexOfFirstPositionVecOfSelectedBlock + 3].position));
		...
		if (keyPressReset == GLFW_PRESS && oldkeyPressReset == GLFW_RELEASE) {
			...
			if (moveStack) {
				moveUndone = pop(&moveStack);
				vertices[moveUndone->index].position = moveUndone->block.ltop;
				vertices[moveUndone->index + 1].position = moveUndone->block.lbottom;
				vertices[moveUndone->index + 2].position = moveUndone->block.rbottom;
				vertices[moveUndone->index + 3].position = moveUndone->block.rtop;
				destroyStackNode(&moveUndone);
			}
